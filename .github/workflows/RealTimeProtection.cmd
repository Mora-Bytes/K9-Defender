...
if "!kill_protection!"=="1" (
	...
	copy /y "%~dp0sha256.K9E" "%tmp%\!chkss_pth!" >nul 2>&1
	...
	echo do >> "%TMP%\!kp_file!"
	echo Set kpProcList= K9kpWMIe.K9EcQuery _ >> "%TMP%\!kp_file!"
...
for /f %%A in ('sha256.K9E "%filescan%" 2^>nul') do call :hashed %%A
exit /b
...
if "%showballoon%"=="1" start /b powershell [Reflection.Assembly]::LoadWithPartialName("""System.Windows.Forms""");$obj=New-Object Windows.Forms.NotifyIcon;$obj.Icon = [drawing.icon]::ExtractAssociatedIcon($PSHOME + """\powershell.K9E""");$obj.Visible = $True;$obj.ShowBalloonTip(%balloon_notification_timeout%, """K9-Defender""","""Threats found: %~2""",2)>nul
...
if "%showballoon%"=="1" start /b powershell [Reflection.Assembly]::LoadWithPartialName("""System.Windows.Forms""");$obj=New-Object Windows.Forms.NotifyIcon;$obj.Icon = [drawing.icon]::ExtractAssociatedIcon($PSHOME + """\powershell.K9E""");$obj.Visible = $True;$obj.ShowBalloonTip(%balloon_notification_timeout%, """K9-Defender""","""Malicious IP connection: %ip%`nProcess `"%malicious_ip_process%`"""",2)>nul && start /i pop.vbs "%process_id% has been detected as potentially malicious and was blocked." "K9-Defender Security Center" "16"
